...
print '**********************************************************************************************************************'
print '*                                                                                                                    *'
print '*           Main function: setting physical host in order to make behaviors of vms on it can be monitored            *'
...
#connect to openstack to collect info of security_vm for every physical host
...
#print dic_mirror_host_bridge_mac#最终用于在物理机上做镜像端口的依据
#setting ovs of each physical host to be mirroring,and the mirroring port is the interface of security_vm
#including 2 command: ovs mirror and bridge transforming to hub
for key in dic_mirror_host_bridge_mac:
    print 'VMs running on physical host '+key+' will be monitored by security_VM: '+dic_mirror_host_bridge_mac[key][0][2]

for key in dic_mirror_host_bridge_mac:
    print '\r\n... ... setting host '+key+' to be monitored ... ...\r\nconnecting to physical host '+key+'.please wait a minute ... ...'
    myssh = pxssh.pxssh()
    ...
    print 'sending mirroring command to physical host '+key+' ... ...'
    command='sudo ovs-vsctl -- set bridge br-int mirrors=@m -- --id=@qvo' + mirrored_port + ' get port qvo' + mirrored_port + ' -- --id=@m create mirror name=mirror0 select-all=true output-port=@qvo' + mirrored_port
    myssh.sendline(command)
    time.sleep(0.5)
    print 'sending brctl command to physical host '+key+' ... ...'
    myssh.sendline('sudo brctl setageing qbr'+mirrored_port+' 0')
    time.sleep(0.5)
    #print command
...
'''
    print s._info
    dic_s=s._info
    print dic_s[u'OS-EXT-SRV-ATTR:host']
    print '/r/n'
    '''
    #print dic_s[u'addresses']

'''
#serv1=nt.servers.find(name='detect-d282876e-f77b-4d24-961b-5134791b1092')
#print serv1
